                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            @if ($element == '...')
                                @if ($paginator->currentPage() > 3)
                                    <a href="{{ $paginator->url(1) }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700  border border-gray-300 cursor-pointer leading-5 bg-orange-200 dark:border-gray-600">1</a>
                                @else
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @elseif ($page >= $paginator->lastPage() - 1) {{-- Mostrar las últimas dos páginas --}}
                                ...
                                @elseif ($page >= $paginator->currentPage() - 2 && $page <= $paginator->currentPage() + 2) {{-- Mostrar solo 2 páginas antes y después de la página actual --}}
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white  border border-black leading-5 hover:text-stone-900 bg-[#a95c2c] focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
