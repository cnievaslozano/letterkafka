            ...
            <div class="flex w-full max-w-screen-lg">
                ...
                <div class="flex flex-col w-28 py-4 pr-3">
                    ...
                    <a class="flex px-3 py-2 mt-auto text-lg rounded-sm font-medium hover:bg-gray-200"
                        ...
                        <div class="flex flex-col ml-2">
                            ...
                            <span
                                class="mt-1 text-xs leading-none">{{ '@' . str_replace(' ', '', Auth::user()->name) }}</span>
                        </div>
                    </a>
                </div>
                {{-- fin menu lateral --}}
                ...
                <div class="flex flex-col flex-grow border-l border-r border-[#493736]">
                    ...
                    <div class="flex-grow h-0 overflow-auto">
                        @if (isset($noCoincidencias) && $noCoincidencias)
                            <div class="text-red-800 text-center my-4 text-xl">No se encontraron resultados para la
                                búsqueda.</div>
                        @elseif (empty($reviews))
                            <div class="text-red-800 text-center my-4 text-xl">No se encontraron resultados</div>
                        @else
                            @foreach ($reviews as $review)
                                <x-kafka.post :user="$review->user" :creation_date="$review->formatearFechaCreacion()" :content="$review" :libro="$review->book"
                                    :userImage="$review->user->profile_photo_path" :likes="$review->likesCount()" :minipost="false" />
                            @endforeach
                            <div class="flex justify-center my-5">
                                <span id="mostrarMas"
                                    class="inline-block bg-br font-extrabold cursor-pointer px-4 py-2">Mostrar mas
                                    reviews</span>
                            </div>
                        @endif
                    </div>
                ...
                <div class="flex flex-col flex-shrink-0 w-1/4 py-4 pl-4">
                    <form action="{{ route('feed.index') }}" method="GET" class="flex items-center" id="searchForm">
                        <input name="username" class="flex items-center h-8 px-2 border border-gray-500 rounded-sm"
                            type="search" Placeholder="nombre de usuario" required>
                        <button type="submit"
                            class="ml-2 bg-[#493736] text-white px-4 py-1 rounded hover:bg-[#493736] h-8">Buscar</button>
                    </form>
                    <div>
                        <h3 class="mt-6 font-semibold">Popular</h3>
                        @foreach ($reviewsPopulares as $popu)
                            <x-kafka.post :user="$popu->user" :creation_date="$popu->creation_date" :content="$popu" :userImage="$popu->user->profile_photo_path"
                                :likes="$popu->likesCount()" :minipost="true" />
                        @endforeach
                        ...
                        @else
                            @foreach ($reviewsAmigos as $amigo)
                                <x-kafka.post :user="$amigo->user" :creation_date="$amigo->creation_date" :content="$amigo" :userImage="$amigo->user->profile_photo_path"
                                    :likes="$popu->likesCount()" :minipost="true" />
                            @endforeach
                        @endif
                    </div>
