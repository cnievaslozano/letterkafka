...
class FeedController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $reviews = Review::with('user', 'book', 'likes')->whereDate('creation_date', '>=', $fecha_limite)->orderByDesc('creation_date')->take(20)->get();

        $reviewsPopulares = Review::with('user', 'book', 'likes')->whereDate('creation_date', '>=', $fecha_limite)->withCount('likes')->orderByDesc('likes_count')->orderBy('creation_date', 'desc')->take(2)->get();
        ...
        if ($request->has('username')) {
            ...
            if ($usuarioBuscado) {
                ...
                $reviews = $usuarioBuscado->reviews()->with('user', 'book', 'likes')->whereDate('creation_date', '>=', $fecha_limite)->orderByDesc('creation_date')->get();
            } else {
        ...
        if ($request->has('reviews_fav')) {
            ...
            $reviews = Review::whereIn('id', $reviewIds)
                ...
                ->orderByDesc(function ($query) {
                    $query->selectRaw('COUNT(*)')->from('likes_reviews')->whereColumn('likes_reviews.review_id', 'reviews.id');
                })
                ->take(10)
                ->get();
        }
